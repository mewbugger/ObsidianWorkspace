...
1. **确保不变性**：`final`字段一旦被赋值后就不可改变，因此通过构造函数传递值并在构造函数内赋值可以确保这一点。这是在对象构建过程中确保数据一致性和不变性的一种方式。
2. **强制初始化**：由于`final`变量必须在构造对象时被初始化（或者在声明时或在初始化块中），提供一个有参构造函数可以强制要求在创建对象时提供必要的参数，这样可以防止忘记设置`final`字段的值，从而避免编译时错误。
3. **灵活性**：相比于在声明时直接初始化，通过构造函数初始化`final`字段提供了更大的灵活性。可以根据不同的构造函数传入不同的值，使得对象的创建更加灵活。
对于上述第二点，代码如下：
``` java
class Person {  
    String name;  
